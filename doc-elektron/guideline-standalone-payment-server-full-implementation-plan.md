...
- [x] `payment_requests` (section 16): `PaymentRequestRepository`, `POST /v1/payment-requests`, `GET /v1/payment-requests/{id}` (public JSON), `GET`/`POST /pay/{id}` (the hosted "confirm or enter an amount, then pay" page), admin UI (create/archive/view spawned orders at `/admin/payment-requests`), spawning a fresh, one-time `orders` row per payment via the same `OrderCreationService` every other order-creation surface uses (`orders.payment_request_id` links it back). ELEK-only for this pass (see `PaymentRequestValidation`'s docblock for why a fiat-denominated request needs its own frozen-rate column this table does not have, unlike a single order). Verified locally: a fixed 2 ELEK request and an open-amount one each spawned a real order with the right amount; a zero/invalid open amount was rejected both client- and server-side; archiving blocked further payments (`GET`/`POST /pay/{id}` both 410/404) while leaving already-spawned orders untouched; the admin detail page listed the spawned order.
- [x] `order_messages` (section 11): `POST`/`GET /v1/orders/{id}/messages` (append-only, server-assigned `created_at`, no update/delete anywhere including the admin UI), scoped `orders:messages` for a Bearer-authenticated merchant OR unauthenticated with the order id itself as the capability token for the buyer (`OrderMessagesController`'s docblock explains why one endpoint pair serves both - $sender is resolved from who is actually calling, never accepted as client input). A matching thread UI on the checkout page (`GET`/`POST /order/{id}/messages`, `CheckoutController`) and the admin order-detail page (`GET`/`POST /admin/orders/{id}/messages`, `DashboardController`) - exactly the same thread, both places, styled per sender. Basic per-order rate limiting (5 messages/60s) shared by both surfaces (`OrderMessageRepository::countRecentByOrder()`). A `window.print()` "Print / export log" button on the admin page satisfies the export/print checklist item with a print stylesheet hiding the nav chrome. Verified locally end-to-end (curl, to sidestep a sandbox-specific Chromium/EventSource-vs-form-navigation interaction that hung real browser form submits on an already-SSE-connected page - a test-environment quirk, not an application bug, confirmed by the identical request working correctly via curl and via a raw `fetch`): a buyer message posted from the checkout page appeared verbatim on the admin order-detail page and vice versa; a Bearer-authenticated GET/POST returned/recorded the same thread with `sender: "merchant"`; a wrong API key was rejected (401); the rate limiter allowed exactly 5 messages then rejected further ones with 422 until the window passed; no PUT/DELETE route exists anywhere for messages.
- [x] Refund flow (section 15): `POST /v1/orders/{id}/refund-address` (JSON, capability-token, no scope) and its HTML-form twin `POST /order/{id}/refund-address` on the checkout page - buyer-supplied, format-validated only (`OrderRefundValidation`: non-empty, no whitespace, fits the column), never checksum-validated (would need bitwasp, and the guideline explicitly doesn't ask for it here). `POST /v1/orders/{id}/mark-refunded` (Bearer, scope `orders:create`) and `POST /admin/orders/{id}/mark-refunded` in the admin order-detail page, both just recording a `marked_refunded` order_event - "self-reported, explicitly, not a verified one" is stated directly in both the admin UI copy and this doc. Also closed section 13's own gap this flow depends on: `Watcher::evaluateOrder()` now flags `overpaid`/`overpayment_lep` on the `settled` event when `received_lep > required_lep`, surfaced as a banner on both the checkout page and the admin order-detail page, linking directly to the refund-address field / mark-refunded button. Verified locally: a whitespace-containing address rejected (422) server-side; a valid one accepted and shown "on file" on the checkout page and in the admin detail grid; the JSON endpoint round-tripped a second address; a real overpayment event (simulated via a direct settled-event insert, same shape the watcher itself would produce) rendered the banner on both pages; marking refunded flipped the admin page to "Marked as refunded (self-reported...)" and appears in the event log; the Bearer endpoint correctly rejected a key missing `orders:create` (403) and an invalid key (401).
