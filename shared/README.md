...
- `Escrow\PlainMultisigEscrowScriptBuilder` is the reference implementation (superseding the earlier `BitwaspEscrowScriptBuilder`, which embedded an order nonce directly in the script and turned out to be unsignable by any real wallet or by bitwasp/bitcoin's own high-level `Signer` -- see that class's docblock). It produces a plain `2 <buyerPubKey> <sellerPubKey> 2 OP_CHECKMULTISIG` script with no nonce and no on-chain timeout enforcement; per-order address uniqueness now comes from `Escrow\XpubChildKeyDeriver` instead (a fresh child pubkey per order, from each party's own connected xpub). See "Escrow address correctness" below for the full list of what this affects.
- `Psbt\ReleasePsbtBuilderInterface`'s reference implementation is `Psbt\Bip174PsbtBuilder`, built on a hand-rolled BIP174 encoder/decoder (`Psbt\Bip174Writer`/`Psbt\Bip174Reader`) since neither bitwasp/bitcoin nor any other PHP library implements PSBT at all (confirmed by exhaustive source search across every released bitwasp/bitcoin version, and by there being no standalone PHP PSBT package on Packagist). Verified against the official BIP174 spec test vector (byte-identical decode/re-encode round trip) and end-to-end against the real bitwasp Script `Interpreter` (build PSBT -> sign with both parties' real keys via the same high-level `Signer` a normal wallet would use -> combined witness independently re-verified by the interpreter). Produces its `$feeRateLepPerVByte` via `ChainData\FeeRateResolver::resolve()` (live mempool estimate, falling back to the admin's fixed `PaymentsConfig::feeRateLepPerVByte()` only when no estimate is available), not by passing that fixed value straight through. `Psbt\PsbtSignatureInspector` validates a PSBT a party hands back before it is relayed to the other one: every field this builder wrote must come back byte-identical, only new signatures under one of the order's own two pubkeys may be added.
